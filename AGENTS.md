...
A **Lawyerify** site — a Hugo static site built on the [Hugolify](https://hugolify.io) module ecosystem. Content is written in Markdown/YAML. Layout comes from Hugo partials. Styling comes from an external styling module (no CSS lives here by default).
